...
const VerbalTestsTab = ({
  ...
  const handleCalculate = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/getAllTQScores`, {
        method: "POST",
        headers: {
      ...
      if (count > 0) {
        ...
        console.log("Final Avg IQ:", avg);
      } else {
        setAverageIQ("");
        console.log("Final Avg IQ: empty (count=0)");
      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {totalVerbalTQScore > 0 && (
                <p
                  ...
                  style={{ color: "var(--dark-gray)" }}
                >
                  Sum of TQ Scores (Standardized): {totalVerbalTQScore.toFixed(2)}
                </p>
              )}
            </div>
          </div>
