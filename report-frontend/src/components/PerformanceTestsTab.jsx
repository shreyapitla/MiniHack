  ...
  const handleCalculate = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/getAllTQScores`,
        {
          method: "POST",
          headers: {
      ...
      if (count > 0) {
        ...
        console.log("Final Avg IQ (Performance):", avg);
      } else {
        setAverageIQ("");
        console.log("Final Avg IQ (Performance): empty (count=0)");
      }
      console.log("Final TQ Scores (Performance):", updatedTqScores);
    ...
    if (!isNaN(verbalIQ) && !isNaN(performanceIQ)) {
      const diff = Math.abs(verbalIQ - performanceIQ).toFixed(2);
      localStorage.setItem("pointsDifference", diff);
    }

    // Helper to classify IQ
    const getClass = (val) => {
      const v = parseFloat(val);
      if (isNaN(v)) return "N/A";
      if (v >= 130) return "Very Superior";
      if (v >= 120) return "Superior";
      if (v >= 110) return "High Average";
      if (v >= 90) return "Average";
      if (v >= 80) return "Low Average";
      if (v >= 70) return "Borderline";
      return "Extremely Low";
    };

    if (!isNaN(verbalIQ)) {
      localStorage.setItem("verbalSuggests", getClass(verbalIQ));
    }
    if (calculatedFinalIQ) {
      localStorage.setItem("finalLevel", getClass(calculatedFinalIQ));
      localStorage.setItem("overallLevel", getClass(calculatedFinalIQ));
    }
  };
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {totalPerformanceTQScore > 0 && (
              <p className="text-lg font-semibold" style={{ color: 'var(--dark-gray)' }}>
                Sum of TQ Scores (Standardized): {totalPerformanceTQScore.toFixed(2)}
              </p>
            )}
          </div>
        </div>
