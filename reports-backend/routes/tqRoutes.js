...
router.post('/getAllTQScores', async (req, res) => {
  ...
  for (const test of tests) {
    ...
    try {
      ...
      if (!mapping) {
        // Find the maximum raw score available in the norms
        const sortedMappings = [...doc.mappings].sort((a, b) => b.raw_score - a.raw_score);
        const maxMapping = sortedMappings[0];

        if (Number(raw_score) > Number(maxMapping.raw_score)) {
          // If raw score > max, use the max TQ score (Requirement: Eliminate N/A)
          results.push({ name: normalizeCanonicalName(usedName), raw_score, tq_score: maxMapping.tq_score });
        } else {
